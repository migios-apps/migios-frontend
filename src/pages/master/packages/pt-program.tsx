      ...
      },
      {
        ...
        cell: ({ row }) => {
          if (row.original.allow_all_trainer) {
            return (
              <Badge
                className={`border ${statusColor["active"]} px-2 py-1 text-xs font-semibold uppercase`}
              >
                All Trainers
              </Badge>
            )
          ...
          const visible = trainers.slice(0, 3)
          const remaining = trainers.length - visible.length

          return (
            <div className="flex items-center gap-2">
              <div className="flex -space-x-2">
                {visible.map((trainer, index) => (
                  <Tooltip key={trainer.id ?? index}>
