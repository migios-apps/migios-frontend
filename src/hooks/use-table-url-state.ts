...
// Helper function to parse URLSearchParams to SearchRecord
export function parseSearchParams(searchParams: URLSearchParams): SearchRecord {
  ...
  searchParams.forEach((_value, key) => {
    ...
    if (allValues.length > 1) {
      ...
      // Single value - try to parse as number if applicable
      const val = allValues[0]
      // Check if it's a number string (for page, pageSize, etc)
...
// Helper function to build URL search string from search record
function buildSearchString(search: SearchRecord): string {
...
export function useTableUrlState(
  ...
  const onPaginationChange: OnChangeFn<PaginationState> = (updater) => {
    ...
    const searchString = buildSearchString(updated)
